...
def joue_joueur(joueur, pioche, tas):
    ...
    if cartes_jouables == []:
        ...
        carte_id = int(input("Choisissez l'id de la carte à jouer"))
        if carte_id not in cartes_jouables:
            print("La carte n'est pas jouable!")
            carte_id = cartes_jouables[random.randint(0, len(cartes_jouables)-1)]    
        print(coulleur_de_baze, "Vous jouez la carte : ")  
        joueur[carte_id].montrer()
        tas.append(joueur.pop(carte_id))
    print(coulleur_de_baze, "-----")
    return joueur, pioche, tas
# à vérifier avec l'affichage, problèmes 
    ...
    while not fin_du_jeu:
        for num, joueur in enumerate(joueurs):
            ...
            if joueur == []:
                ...
                fin_du_jeu = True
                break
            if num == 0: # temporaire, juste pour choisir quel joueur est vraiment joueur et le differenttier des bots, enlever cette ligne fait joueur les bots entre eux
                joueur, pioche, tas = joue_joueur(joueur, pioche, tas)
